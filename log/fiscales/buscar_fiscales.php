...
if (isset($_POST)) {
 ...
 <center><h2>Fiscales <?=$sedes_nom?></h2></center>
    ...
    if ($conn) {
        ...
        if ($sedes_nom) {
                ...
                if ($fecha_desde && $fecha_hasta) {
                    ...
                    $sql="SELECT * from fiscal WHERE tienda='$sedes_nom'  order by fis_fecha desc ";

                } elseif ($fecha_desde && !$fecha_hasta) {

                    $sql="SELECT * from fiscal WHERE tienda='$sedes_nom' and fis_fecha >='$fecha_desde'  order by fis_fecha desc ";

                }elseif (!$fecha_desde && $fecha_hasta) {

                    $sql="SELECT * from fiscal WHERE tienda='$sedes_nom' and fis_fecha  <= '$fecha_hasta' order by fis_fecha desc ";
                ...
                if ($fecha_desde && $fecha_hasta==null) {
                    ...
                    $sql="SELECT * from fiscal WHERE fis_fecha >='$fecha_desde'  order by fis_fecha desc ";
                }elseif ($fecha_desde==null && $fecha_hasta) {


                    $sql="SELECT * from fiscal WHERE fis_fecha <='$fecha_hasta'  order by fis_fecha desc ";
                }
