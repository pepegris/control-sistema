...
if (isset($_POST['scripts']) && isset($_POST['clave'])) {
    ...
    // =================================================================
    // PREPARAR EL MONITOR (ESTRATEGIA INFALIBLE: IDs)
    // =================================================================
    ...
    if ($script == 'backups') {
        ...
        // Conectar para buscar el último backup_set_id
        ...
        // --- RESTORE (Servidor .19 - Integración) ---
        // CAMBIA LA IP SI TU INTEGRACIÓN ESTÁ EN OTRO LADO
        $serverIP = "172.16.1.19"; // <--- IP DE DONDE SE RESTAURA
        $monitorURL = "../../services/adm/cob-eg-ig/check_restore.php";
        
        // Conectar para buscar el último restore_history_id
        $connectionInfo = array("Database" => "msdb", "UID" => "mezcla", "PWD" => "Zeus33$", "CharacterSet" => "UTF-8");
        $connMonitor = sqlsrv_connect($serverIP, $connectionInfo);
        
        if ($connMonitor) {
            $sql = "SELECT ISNULL(MAX(restore_history_id), 0) as dato FROM msdb.dbo.restorehistory";
            $stmt = sqlsrv_query($connMonitor, $sql);
            if ($stmt && $row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
                $lastID = $row['dato'];
            }
    ...
    // El parámetro siempre es last_id, funciona para ambos casos
...
<style>
    ...
    .btn-primary { background-color: #0984e3; border: none; padding: 10px 20px; color: white; text-decoration: none; border-radius: 5px; font-size: 1.1em; }
...
<div class="container">
    <div class="status-card">
        ...
        <div id="resultSection" style="display: none;">
            ...
            <br><br><a href="Import-database.php" class="btn btn-primary">Volver al Menú</a>
        </div>
...
<script>
    ...
    const monitorParams = "<?= $monitorParam ?>"; 
    ...
    function mostrarResultado(status) {
        setTimeout(() => {
            ...
            resultMsg.innerText = 'Operación completada exitosamente.';
        }, 1000);
    }
